...
static struct pes_t* ps_stream_find(struct ps_muxer_t *ps, int codecid)
{
    size_t i;
    for (i = 0; i < ps->psm.stream_count; i++)
    {
        if (codecid == ps->psm.streams[i].codecid)
            return &ps->psm.streams[i];
    }
    return NULL;
}

int ps_muxer_input(struct ps_muxer_t* ps, int codecid, int flags, int64_t pts, int64_t dts, const void* data, size_t bytes)
{
	int first;
	size_t i, n, sz;
	uint8_t *packet;
    ...
    stream = ps_stream_find(ps, codecid);
